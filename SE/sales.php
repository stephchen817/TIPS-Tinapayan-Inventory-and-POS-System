        ...
        <main>
            <section class='sec-1' id='panel'>
                <div class='container-xl'>
                    ...
                    <center>
                        <h2 class='loggedIn' style='font-family:Baloo Tamma; font-size:25px'id='username'><?php echo $_SESSION["loginUser"]; ?></h3>
                    </center>
                        <div class = "row no-gutters">
                            <div class = "col mr-1">
                                <button id='add' type="button" name="add" class="btn-block btn-info btn-primary">
                                    Add
                                </button>
                            </div>
                            <div class = "col">
                            <form method="post" id="user_form">
                                <input type="submit" name="insert" id="insert" class="btn btn-block btn-primary" value="Save" style="margin-top: 0px;"/>
                            </form>
                            </div>
                        </div>
                    <button onclick="window.location.href='invoiceDetails.php'" id='return' type="button" class="btn-block btn-info btn-secondary">
                        View Transaction
                    </button>
...
<script>
    $(document).ready(function() {
        ...
        $('#add').click(function() {
            $('#user_dialog').dialog('option', 'title', 'Insert transaction');
            $('#invoiceID').val('');
            $('#item_name').val('');
            $('#productqty').val('');
        ...
        $('#save').click(function() {
            ...
            if (error_invoiceID != '' || error_item_name != '' || error_productqty != '') {
                ...
                if ($('#save').text() == 'Save') {
                    ...
                    output += '<td align="center"><button type="button" name="view_details" class="btn btn-xs view_details" id="' + count + '">View</button></td>';
                    output += '<td align="center"><button type="button" name="remove_details" class="btn btn-xs remove_details" id="' + count + '">Remove</button></td>';
                    ...
                    output = '<td align="center">' + invoiceID + ' <input type = "hidden" name = "hidden_invoiceID[]" id = "invoiceID' + row_id + '" class = "invoiceID" value ="' + invoiceID + '"/></td>';

                    output += '<td align="center">' + item_name + ' <input type = "hidden" name = "hidden_item_name[]" id = "item_name' + row_id + '" class = "item_name" value ="' + item_name + '"/></td>';

                    output += '<td align="center">' + productqty + ' <input type = "hidden" name = "hidden_productqty[]" id = "productqty' + row_id + '" class = "productqty" value ="' + productqty + '"/></td>';

                    output += '<td align="center"><button type="button" name="view_details" class="btn btn-warning btn-xs view_details" id="' + row_id + '">View</button></td>';
                    output += '<td align="center"><button type="button" name="remove_details" class="btn btn-danger btn-xs remove_details" id="' + row_id + '">Remove</button></td>';
